...
from utils.id_utils import clean_id, extract_id_parts
...
def load_employee_master(emp_path: str) -> pd.DataFrame:
    ...
    if any(x is None for x in need):
        raise RuntimeError("Faltan columnas en datos empleados.xlsx: EMPRESA, SECTOR, ID, APELLIDO Y NOMBRE, JORNADA, $/hs LaV, $/hs Sab, $/hs DomYFer.")

    out = pd.DataFrame()
    out["id_master_display"] = df[c_id].apply(clean_id).astype(str).str.strip()
    parts = out["id_master_display"].apply(lambda x: pd.Series(extract_id_parts(x), index=["id_key", "id_digits", "id_cuil11", "id_dni8"]))
    ...
    out = out[out["id_key"] != ""].copy().reset_index(drop=True)
    out["midx"] = out.index
    return out